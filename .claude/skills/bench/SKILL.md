---
name: bench
description: Roda o agente Kaggriculture (main.py) várias vezes contra o motor oficial do jogo e reporta o saldo final médio/mínimo/máximo. Use sempre que uma mudança de lógica em main.py precisar ser validada antes de ser considerada uma melhoria.
---

Este projeto tem uma regra de ouro (ver `CLAUDE.md`): nenhuma mudança em
`main.py` deve ser tratada como melhoria sem medição real contra o motor
...
4. Reporte ao usuário: saldo médio, mínimo, máximo, e quantas partidas (se
   ...
   não apenas de resultado ruim (ver rodada 4 do
   `docs/DEVELOPMENT_LOG.md` para um exemplo real).
